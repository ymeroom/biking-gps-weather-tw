...
- **橘色點（+10/+20/+30 分）**：照你現在的速度推算的前方位置，拿去對照雷達回波，
  目測「我到那裡時雨帶到哪了」。靜止時不顯示。
  - **實線外框＝沿路線推**：選了路線、人也真的騎在線上（離線 150 m 內）時，點是沿著路線往前走的，
    過彎不會飛到河對岸。逆向騎也認得（比對行進方位與路段方位）。
    快騎到路線頭尾時，推出去已在路線外的那幾個點會自動改回直線推估（外框跟著變虛線）。
  - **虛線外框＝直線推估**：沒選路線、或人不在路線上時，照目前方位拉直線的粗估。
...
- **固定路線**：`routes/*.json`（`scripts/gpx_to_routes.py` 把實騎 GPX 抽稀轉出，靜態檔、不進 `data` 分支）。
  ...
  前方推算點也吃這份路線：`snapToRoute()` 把定位投影到折線上（等距圓柱近似，逐段算垂足），
  再用累積距離沿線前推。**搜尋範圍會先限縮在上次吸附位置前後 2 km**——淡水河兩岸的車道只隔一百多公尺，
  單純取全線最近點會讓前方點在對岸跳（實測輕鬆騎路線有一對點相距 101 m、沿線卻差 95 km）。
...
- `QPF_SCALE` 改成單一來源（現在 `index.html` 跟 `scripts/fetch_cwa.py` 各一份，靠註解提醒同步）
